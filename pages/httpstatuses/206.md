...
The server is successfully fulfilling a range request for the target resource by transferring one or more parts of the selected representation that correspond to the satisfiable ranges found in the request's Range header field[1](#ref-1).
...
Within the header area of each body part in the multipart payload, the server MUST generate a Content-Range header field corresponding to the range being enclosed in that body part. If the selected representation would have had a Content-Type header field in a [200 OK](https://httpstatuses.com/200) response, the server SHOULD generate that same Content-Type field in the header area of each body part. For example:
...
When a 206 response is generated, the server MUST generate the following header fields, in addition to those required above, if the field would have been sent in a [200 OK](https://httpstatuses.com/200) response to the same request: Date, Cache-Control, ETag, Expires, Content-Location, and Vary.

If a 206 is generated in response to a request with an If-Range header field, the sender SHOULD NOT generate other representation header fields beyond those required above, because the client is understood to already have a prior response containing those header fields. Otherwise, the sender MUST generate all of the representation header fields that would have been sent in a [200 OK](https://httpstatuses.com/200) response to the same request.

A 206 response is cacheable by default; i.e., unless otherwise indicated by explicit cache controls[3](#ref-3).

* * *

*   1 Range [RFC7233 Section 3.1](https://tools.ietf.org/html/rfc7233#section-3.1)
*   2 Internet Media Type multipart/byteranges [RFC7233 Appendix A](https://tools.ietf.org/html/rfc7233#appendix-A)
*   3 Calculating Heuristic Freshness [RFC7234 Section 4.2.2](https://tools.ietf.org/html/rfc7234#section-4.2.2)
*   Source: [RFC7233 Section 4.1](https://tools.ietf.org/html/rfc7233#section-4.1)
