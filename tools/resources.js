...
const { getYouTubeVideoInfo } = require("./yt");

// FUNCTIONS: ==================================================================
function findYouTubeById(vid) {
...
if (Object.keys(args.values).length === 0) {
  warn("Missing arguments. Maybe you wanted to run with:");
  Object.entries(options).forEach(([key, value]) => {
    info(`--${key}${value.__arg ? " " + value.__arg : ""} or -${value.short} ${value.__arg ? value.__arg : ""}`);
  })
...
const vidToAdd = args.values["add-youtube-by-id"];
...
// ADD YOUTUBE BY ID
if (vidToAdd && typeof vidToAdd === "string") {
  ...
  getYouTubeVideoInfo({ vid: vidToAdd })
  .then( data =>{

    appendObjectToResources(JSON.parse(`{ ${data.output} }`));
